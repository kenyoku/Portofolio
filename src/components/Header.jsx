...
import{FiGithub,FiTwitter,FiInstagram,FiLinkedin,FiMenu,FiX} from "react-icons/fi";
...
const Header = () => {
  ...
  const closeContatForm=()=>setContactFormOpen(false);
  ...
  return (
    ...
    <div className="h-10 w-10 rounded-x1 bg-gradient-to-r from-gray-500 to-gray-100 flex items-center justify-center text-purple-600 font-bold text-xl mr-3">MS</div>
    <span className="text-xl font-bold bg-gradient-to-r from-gray-300 to-gray-100 bg-clip-text text-transparent">Malicode</span>
    </motion.div> 
    ...
    <nav className="lg:flex hidden space-x-8">
      {["Home", "About", "Projects", "Experience", "Contact"].map((item, index)=>(
        ...
        className="relative text-gray-800 text-white dark:text-gray-200 hovor:violet-600
   ...
   <div className="md:flex hidden items-center space-x-4">
    ...
    transitin={{delay:1.3,duration:0.8}}
   ...
   onClick={openContactForm}
    initial={{opacity:0,sclae:0.8}}
    animate={{opacity:1,scale:1}}
    transitin={{delay:1.6,duration:0.8,type:"spring",stiffness:100,damping:15}}
...
{/*Mobile Menu*/}
<motion.div
initial={{opacity:0,sclae:0}}
animate={{opacity:isOpen ? 1:0,
          height:isOpen ? "auto":0}}
transitin={{duration:0.5}}
className="md:hidden overflow-hidden be-white dark:bg-gray-900 shadow-lg px-4 py-5 space-y-5">
  <nav className="flex flex-col space-y-3">
  {["Home", "About", "Projects", "Experience", "Contact"].map((item) =>
  (
    <a className="text-gray-300 font-medium py-2" key={item} href="#">{item}</a>
  ))}
  </nav>
...
{contactFormOpen &&(
  ...
  items-center jusify-center p-4"
  ...
  >
    ...
    className="bg-white dark:bg-gray-800 rounded-x1 shadow-xl w-full max-w-md p-6">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2x1 font-bold text-gray-300">Get In Touch</h1>
